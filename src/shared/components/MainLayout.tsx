...
export default function MainLayout({ children }: MainLayoutProps) {
  ...
  const renderNavLink = (l: (typeof NAV_LINKS)[number]) => {
    ...
    if (isRoute) {
      return (
        <Link key={l.href} to={l.href} onClick={() => setMobileOpen(false)} className="nav-link">
          {l.label}
        </Link>
      )
    }
    return (
      <a key={l.href} href={l.href} onClick={() => setMobileOpen(false)} className="nav-link">
        {l.label}
      </a>
    )
  }
      ...
      <header className={`fixed z-50 transition-all duration-500 ease-in-out ${
        ...
        <div className={`${layout.container} flex items-center justify-between py-3 sm:py-4`}>
          ...
          <nav className="hidden items-center gap-8 md:flex">
            ...
            >
              <button type="button" className="nav-link inline-flex items-center gap-1" onClick={() => setServicesOpen((o) => !o)}>
                Servicios
          ...
          >
            <nav className="flex flex-col gap-3 pt-3">
              ...
              <div className="flex items-center gap-4">
                <a href="https://instagram.com" target="_blank" rel="noopener noreferrer" onClick={() => setMobileOpen(false)}
                ...
                <a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" onClick={() => setMobileOpen(false)}
                  className="flex items-center gap-2 text-sm transition-colors"
                  style={{ color: headerActive ? colors.tealDeep : 'rgba(255,255,255,0.7)' }}
                >
                  <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
